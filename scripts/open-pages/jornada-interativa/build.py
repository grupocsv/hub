...
def export_svg(svg, out, asset_directory=None):
    ...
    for element in root.iter():
        if element.tag.endswith('}image'):
            ...
            if asset_directory and href.startswith('https://open.grupocsv.com/jornada-tea/'):
                # Snapshot autenticado do prefixo; não contorna o gate com credenciais em URL.
                base=asset_directory.resolve()
                relative=urlsplit(href).path.removeprefix('/jornada-tea/')
                target=(base/relative).resolve()
                require(target.is_relative_to(base),'Caminho de asset local inválido')
                raw=target.read_bytes()
                types={'.png':'image/png','.svg':'image/svg+xml','.webp':'image/webp','.jpg':'image/jpeg','.jpeg':'image/jpeg'}
