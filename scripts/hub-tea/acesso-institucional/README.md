...
## Revisão: calculadora relacionada à Jornada

`calculator_related.py` substitui a entrada no cabeçalho por um complemento imediatamente abaixo do card Jornada, no computador e no celular. O nome completo, a descrição breve e a ação ficam sobre o fundo da página, sem numeração, selo ou moldura de quinto card. Os quatro cards e seus conteúdos são preservados literalmente; o único destino da calculadora permanece o mesmo.

O build recebe em `--source-package` o pacote publicado pela PR161. A publicação recebe ainda `--labels-package` e `--base-package` da cadeia anterior, além de snapshot e estado privados. Herda a mesma restrição de escrita a `tea/index.html`, exige o manifesto `calculator-related-approved.json` revisado, valida a origem antes de escrever e guarda `before-related.html`. Não altera scripts de autenticação, Worker ou assets. Não executar o publicador anterior contra esta nova versão.

Usar `verify-browser.mjs PACOTE SAIDA --restricted-cards --calculator-related`, com `--live` apenas após publicação. A prova inclui ausência de link duplicado no cabeçalho, relação espacial entre Jornada e calculadora, texto inteiro, alvo de toque de 44 pixels e sequência de teclado Jornada → calculadora → Painel → Relatório, além das verificações já existentes.
