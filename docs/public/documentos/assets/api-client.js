...
const ERROR_DEFINITIONS = Object.freeze({
  invalid_request_target: Object.freeze({ category: 'client', retriable: false }),
  forbidden_client_context: Object.freeze({ category: 'client', retriable: false }),
  forbidden_client_header: Object.freeze({ category: 'client', retriable: false }),
  session_required: Object.freeze({ category: 'authentication', retriable: false }),
  session_expired: Object.freeze({ category: 'authentication', retriable: false }),
  resource_unavailable: Object.freeze({ category: 'neutral', retriable: false }),
  conflict: Object.freeze({ category: 'conflict', retriable: false }),
  payload_too_large: Object.freeze({ category: 'validation', retriable: false }),
  unsupported_media_type: Object.freeze({ category: 'validation', retriable: false }),
  ...
  service_unavailable: Object.freeze({ category: 'transient', retriable: true }),
  ...
  network_unavailable: Object.freeze({ category: 'transient', retriable: true }),
...
const PUBLIC_MESSAGES = Object.freeze({
  ...
  forbidden_client_context: 'A solicitação local contém um contexto não permitido.',
  forbidden_client_header: 'A solicitação local contém um cabeçalho não permitido.',
...
export class DocumentApiError extends Error {
  constructor({ status = 0, code = 'unexpected_response', requestId = null, retryAfterSeconds = null } = {}) {
    const normalizedCode = Object.hasOwn(ERROR_DEFINITIONS, code) ? code : 'unexpected_response';
...
function hasForbiddenContext(value, seen = new Set()) {
  ...
  if (Array.isArray(value)) return value.some((item) => hasForbiddenContext(item, seen));
  ...
  return Object.entries(value).some(
    ([key, child]) => FORBIDDEN_CONTEXT_KEYS.has(key.toLowerCase()) || hasForbiddenContext(child, seen),
  );
}
...
function resolveTarget(baseUrl, target) {
  ...
  if (resolved.origin !== baseUrl.origin || !resolved.pathname.startsWith('/v1/')) {
    throw clientError('invalid_request_target');
  }
...
function prepareHeaders(inputHeaders, token, hasJsonBody) {
...
function prepareBody(body) {
  if (body === undefined || body === null) return { body: undefined, hasJsonBody: false };
...
function codeForStatus(status) {
  ...
  if (status === 400 || status === 411 || status === 422) return 'invalid_request';
...
function createAbortContext(callerSignal, timeoutMs, setTimer, clearTimer) {
  ...
  const abortFromCaller = () => {
    callerAborted = true;
    controller.abort(callerSignal?.reason || new DOMException('Cancelado', 'AbortError'));
  };
  ...
  return {
    ...
    resultCode() {
      return timedOut ? 'request_timeout' : callerAborted ? 'request_aborted' : 'request_aborted';
    },
    dispose() {
      clearTimer(timer);
...
export function publicErrorState(error) {
  if (error instanceof DocumentApiError && error.code === 'resource_unavailable') {
  ...
  const canRetry = error instanceof DocumentApiError && error.retriable === true;
  return Object.freeze({
    state: 'error',
...
export function createDocumentApiClient(options = {}) {
  ...
  if (typeof getSession !== 'function' || typeof fetchImpl !== 'function') {
    throw new TypeError('Dependências obrigatórias do cliente documental estão ausentes.');
  }
  ...
  async function request(target, requestOptions = {}) {
    ...
    const headers = prepareHeaders(requestOptions.headers, session.token, preparedBody.hasJsonBody);
