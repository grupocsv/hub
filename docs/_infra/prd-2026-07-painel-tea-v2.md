...
- **Pública** — `open.grupocsv.com/painel-tea/` (via `open_page_publish`): **gate embutido** com marcador `id="gate"` (impede injeção dupla pelo Worker), `POST https://csv-open-auth.guilherme-thom.workers.dev/auth` com `PAGE='painel-tea'`, senha compartilhada + e-mails `@unimedgv.com.br`/`.coop.br`, sessão de 4h, notificação de acesso, "trancada desde o primeiro byte"; **sem** hub-auth e sem link para o hub; assets absolutizados (`https://hub.grupocsv.com/...` e `assets.grupocsv.com`); não listada no menu do portal (decisão consciente pré-existente). Como o conteúdo já é 100% agregado e k-anonimizado, o gate é defesa em profundidade — não a única barreira.
